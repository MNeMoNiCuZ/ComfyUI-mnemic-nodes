...
import { chainCallback, addMiddleClickPan, addWheelPassthrough, cursorForBboxMode, watchImageInputs, captureVideoFrame } from './ideogram4_utility.js';

// Ideogram 4 Prompt Builder editor (adapted from KJNodes' Ideogram4PromptBuilderKJ by kijai).
// MNeMiC additions: a "+ Region" toolbar button, and one dynamic region_N STRING
// input pin per region (a connected string overrides that region's description).
...
const REGION_INPUT_RE = /^region_\d+$/;
...
function injectStyle() {
  if (document.getElementById("mnideo-style")) return;
  const s = document.createElement("style");
  s.id = "mnideo-style";
  s.textContent = `
    .mnideo-wrap { display:flex; flex-direction:column; overflow:hidden; position:relative; pointer-events:auto; gap:4px; }
    .mnideo-canvas { cursor:crosshair; display:block; width:100%; height:auto; flex:0 0 auto; background:#1a1a1a; border-radius:4px; outline:none; }
    .mnideo-bar { display:flex; align-items:center; gap:6px; font:11px sans-serif; color:#aaa; user-select:none; padding:0 2px; flex:0 0 auto; }
    .mnideo-panel { display:flex; flex-direction:column; gap:5px; padding:6px; background:#262626; border-radius:4px; font:11px sans-serif; color:#bbb; flex:0 0 auto; }
    .mnideo-row { display:flex; align-items:center; gap:6px; flex-wrap:wrap; }
    .mnideo-btn { background:#333; border:1px solid #555; border-radius:4px; color:#bbb; font:11px sans-serif; cursor:pointer; padding:2px 8px; line-height:16px; white-space:nowrap; flex-shrink:0; }
    .mnideo-btn:hover { border-color:#46b4e6; color:#fff; }
    .mnideo-btn.active { border-color:#46b4e6; color:#46b4e6; background:#2a3a42; }
    .mnideo-area { width:100%; box-sizing:border-box; background:#1d1d1d; border:1px solid #444; border-radius:4px; color:#ddd; font:13px monospace; padding:4px 6px; resize:vertical; min-height:36px; }
    .mnideo-sw { width:20px; height:20px; border:1px solid #666; border-radius:3px; cursor:pointer; flex-shrink:0; position:relative; transition:transform .18s ease, box-shadow .12s ease, opacity .12s ease; }
    .mnideo-sw:hover { transform:scale(1.2); box-shadow:0 0 0 2px #46b4e6; z-index:3; }
    .mnideo-sw.dragging { opacity:.4; box-shadow:0 0 0 2px #46b4e6; }
    body.mnideo-dragging, body.mnideo-dragging * { cursor:move !important; }
    .mnideo-sw input { position:absolute; opacity:0; width:0; height:0; pointer-events:none; }
    .mnideo-inline { position:absolute; box-sizing:border-box; background:rgba(18,18,18,0.92); border:2px solid #46b4e6; border-radius:3px; color:#fff; font:13px monospace; padding:3px 4px; resize:none; outline:none; z-index:10; }
    .mnideo-menu { position:fixed; z-index:10000; background:#262626; border:1px solid #555; border-radius:6px; padding:4px; box-shadow:0 6px 20px rgba(0,0,0,0.55); font:12px sans-serif; color:#ddd; max-height:60vh; overflow-y:auto; min-width:210px; max-width:340px; }
    .mnideo-mhdr { font:11px sans-serif; color:#888; padding:2px 6px 4px; user-select:none; }
    .mnideo-lrow { display:flex; align-items:center; gap:6px; padding:3px 5px; border-radius:4px; cursor:move; user-select:none; transition:transform .18s ease, box-shadow .12s ease, opacity .12s ease, background .12s; }
    .mnideo-lrow:hover { background:#333; }
    .mnideo-lrow.active { background:#2a3a42; box-shadow:inset 0 0 0 1px #46b4e6; }
    .mnideo-lrow.dragging { opacity:.4; box-shadow:0 0 0 2px #46b4e6; background:#333; }
    .mnideo-lsw { width:16px; height:16px; border-radius:3px; border:1px solid #666; flex:0 0 auto; }
    .mnideo-lnum { font:bold 11px monospace; color:#888; flex:0 0 auto; width:18px; }
    .mnideo-ltext { flex:1 1 auto; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .mnideo-ltext.empty { color:#777; font-style:italic; }
    .mnideo-lbtn { background:none; border:none; color:#999; cursor:pointer; font:13px sans-serif; line-height:1; padding:2px 5px; border-radius:3px; flex:0 0 auto; }
    .mnideo-lbtn:hover { color:#fff; background:#444; }
    .mnideo-lbtn.del:hover { color:#fff; background:#a33; }
    .mnideo-note { font:10px sans-serif; color:#777; }
  `;
  document.head.appendChild(s);
}

app.registerExtension({
  name: "mnemic.Ideogram4PromptBuilder",
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    chainCallback(nodeType.prototype, "onNodeCreated", function () {
      ...
      // Show photo/art_style widgets only when the matching style kind is selected.
      function setWidgetVisible(w, vis) {
        if (!w) return;
        if (vis) { w.hidden = false; delete w.computeSize; }
        else { w.hidden = true; w.computeSize = () => [0, -4]; }
      }
      function updateStyleWidgets() {
        const kind = findW("style")?.value ?? "none";
        setWidgetVisible(findW("photo"), kind === "photo");
        setWidgetVisible(findW("art_style"), kind === "art_style");
      ...
      function syncRegionInputs() {
        ...
        let have = node.inputs.filter((inp) => REGION_INPUT_RE.test(inp.name)).length;
        while (have < want) {
          have++;
          node.addInput(`region_${have}`, "STRING");
        }
      ...
      wrap.className = "mnideo-wrap";
      const bar = document.createElement("div");
      bar.className = "mnideo-bar";
      const hint = document.createElement("span");
      hint.style.flex = "1";
      const addRegionBtn = document.createElement("button");
      addRegionBtn.className = "mnideo-btn";
      addRegionBtn.textContent = "+ Region";
      addRegionBtn.title = "Add a new region (also adds a region_N string input pin)";
      const copyBtn = document.createElement("button");
      copyBtn.className = "mnideo-btn";
      copyBtn.textContent = "Copy";
      copyBtn.title = "Copy the current caption JSON to the clipboard";
      const importBtn = document.createElement("button");
      importBtn.className = "mnideo-btn";
      ...
      // "+ Region": add a default box (staggered so stacked adds stay visible).
      function addRegion() {
        closeInlineEditor();
        const k = node._boxes.length % 6;
        node._boxes.push({ x: 0.05 + k * 0.05, y: 0.05 + k * 0.05, w: 0.3, h: 0.3, type: "obj", text: "", desc: "", palette: [] });
        node._activeIdx = node._boxes.length - 1;
        commit(); fitNode();
        canvasEl.focus();
      }
      stopProp(addRegionBtn);
      ...
      function openLayersMenu(clientX, clientY) {
        ...
        menu.className = "mnideo-menu";
        const hdr = document.createElement("div");
        hdr.className = "mnideo-mhdr";
        ...
        const renumber = () => Array.from(list.querySelectorAll(".mnideo-lrow")).forEach((row, k) => {
          row.querySelector(".mnideo-lnum").textContent = String(k + 1).padStart(2, "0");
        });
        function buildRows() {
          list.innerHTML = "";
          if (!node._boxes.length) {
            const empty = document.createElement("div");
            empty.className = "mnideo-mhdr"; empty.textContent = "No regions yet.";
            list.appendChild(empty);
            return;
          }
          node._boxes.forEach((b, i) => {
            const row = document.createElement("div");
            row.className = "mnideo-lrow" + (i === node._activeIdx ? " active" : "");
            row._box = b;
            const sw = document.createElement("div");
            sw.className = "mnideo-lsw";
            sw.style.background = (b.palette || []).find(Boolean) || "#8c8c8c";
            const num = document.createElement("span");
            num.className = "mnideo-lnum"; num.textContent = String(i + 1).padStart(2, "0");
            const txt = document.createElement("span");
            const label = rowLabel(b);
            txt.className = "mnideo-ltext" + (label ? "" : " empty");
            txt.textContent = label || (b.type === "text" ? "(text)" : "(empty)");
            txt.title = label;
            const dup = document.createElement("button");
            dup.className = "mnideo-lbtn"; dup.textContent = "⧉";
            dup.title = "Duplicate, then click on the canvas to place";
            const del = document.createElement("button");
            del.className = "mnideo-lbtn del"; del.textContent = "✕";
            ...
            row.addEventListener("click", () => {
              ...
              commit();
              for (const r of list.querySelectorAll(".mnideo-lrow")) r.classList.toggle("active", r._box === b);
            });
            ...
            row.addEventListener("mousedown", (e) => {
              ...
              const move = (me) => {
                if (!dragging) {
                  ...
                  dragging = true; row.classList.add("dragging"); document.body.classList.add("mnideo-dragging");
                }
                for (const other of list.querySelectorAll(".mnideo-lrow")) {
                  ...
                  if (me.clientY >= r.top && me.clientY <= r.bottom) {
                    ...
                    const els = Array.from(list.querySelectorAll(".mnideo-lrow"));
              ...
              const up = () => {
                ...
                document.body.classList.remove("mnideo-dragging");
                if (dragging) {
                  row.classList.remove("dragging");
                  row._dragged = true;                             // suppress the trailing click
                  const active = node._boxes[node._activeIdx];
                  const order = Array.from(list.querySelectorAll(".mnideo-lrow")).map((el) => el._box);
      ...
      function buildCaption() {
        ...
        if (kind !== "none") {
          ...
          if (kind === "photo") { sd.photo = getW("photo") || ""; sd.medium = getW("medium"); }
      ...
      function applyCaption(cap) {
        ...
        if (kind === "photo") setWidgetVal("photo", sd.photo || "");
        else if (kind === "art_style") setWidgetVal("art_style", sd.art_style || "");
      ...
      function buildSwatchRow(container, arr, max, onEdit, onStruct) {
        arr.forEach((hex, i) => {
          ...
          sw.addEventListener("mousedown", (e) => {
            ...
            const move = (me) => {
              if (!dragging) {
                if (Math.abs(me.clientX - sx) + Math.abs(me.clientY - sy) < 4) return;
                dragging = true; sw.classList.add("dragging"); document.body.classList.add("mnideo-dragging");
              }
              for (const other of container.querySelectorAll(".mnideo-sw")) {
                ...
                if (me.clientX >= r.left && me.clientX <= r.right && me.clientY >= r.top - 6 && me.clientY <= r.bottom + 6) {
                  ...
                  const els = Array.from(container.querySelectorAll(".mnideo-sw"));
            ...
            const up = () => {
              ...
              if (dragging) {
                ...
                const order = Array.from(container.querySelectorAll(".mnideo-sw")).map((el) => el.dataset.hex);
        ...
        if (arr.length < max) {
          ...
          add.className = "mnideo-btn"; add.textContent = "+";
          stopProp(add);
      ...
      function renderPanel() {
        ...
        const b = node._boxes[node._activeIdx];
        if (!b) {
          hint.textContent = "Drag on the canvas (or + Region) to add a region";
        ...
        hintB.style.color = col;
        hintB.textContent = `region ${node._activeIdx + 1}`;
        hint.appendChild(hintB);
        hint.appendChild(document.createTextNode(" · dbl-click edit · alt-click overlap · del remove"));

        // type toggle
        const typeRow = document.createElement("div");
        typeRow.className = "mnideo-row";
        const lbl = document.createElement("span"); lbl.textContent = "type:"; typeRow.appendChild(lbl);
        for (const t of ["obj", "text"]) {
          const btn = document.createElement("button");
          btn.className = "mnideo-btn" + (b.type === t ? " active" : "");
          btn.textContent = t;
          stopProp(btn);
      ...
      for (const name of ["background", "high_level_description", "aesthetics", "lighting", "medium", "photo", "art_style"]) {
        const w = findW(name);
        if (w) chainCallback(w, "callback", () => updateTokens());
      }
      // The style kind switches which descriptor widget is shown (and changes the JSON).
      const styleKindW = findW("style");
      if (styleKindW) chainCallback(styleKindW, "callback", () => { updateStyleWidgets(); fitNode(); updateTokens(); });
