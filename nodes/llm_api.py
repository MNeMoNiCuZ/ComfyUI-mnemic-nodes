...
def _log(message):
    print(f"\033[36m[MNeMiC LLM]\033[0m {redact(message)}")
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input("reasoning", options=REASONING_LEVELS, default="default", advanced=True,
                               tooltip="How hard a reasoning model should think. 'default' sends nothing; 'none' turns thinking off where supported. Claude maps this to an extended-thinking token budget."),
                io.Int.Input("max_tokens", default=0, min=0, max=262144, step=1, advanced=True,
                             tooltip="Maximum length of the reply in tokens. 0 leaves it to the server (Claude, which needs a value, gets 4096)."),
