"""Ideogram 4 prompt builder with a visual bbox editor.

Adapted from KJNodes' Ideogram4PromptBuilderKJ (by kijai), converted to the
classic node API and extended with dynamic per-region STRING inputs: every
region drawn in the editor exposes a matching `region_N` input pin whose
connected string overrides that region's description.

Draw regions on the canvas (or press "+ Region"), set each region's
type/desc/text/color palette, and assemble the Ideogram 4 JSON caption prompt.
"""
...
class Ideogram4PromptBuilder:
    ...
    Drag on the canvas (or press "+ Region") to add regions; select a region to
    set its type (obj/text), description, text, and color palette. Each region
    exposes a region_N STRING input pin — when connected, the incoming string
    overrides that region's description. Outputs the assembled caption JSON.
    """
    ...
    def build_prompt(self, width, height, high_level_description="", background="", style="none",
        ...
        # a connected, non-empty string overrides that region's description.
        for i, box in enumerate(boxes):
            override = kwargs.get("region_%d" % (i + 1))
            if isinstance(override, str) and override.strip() and isinstance(box, dict):
                box["desc"] = override
